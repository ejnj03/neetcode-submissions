class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        po = deque([(0, c) for c in range(nc)] + [(r, 0) for r in range(nr)])
        ao = deque([(nr - 1, c) for c in range(nc)] + [(r, nc - 1) for r in range(nr)])
        ...
        for oc, valid in [(po, pv), (ao, av)]:
            while len(oc) > 0: 
                r, c = oc.popleft()
                if (r, c) in valid: continue
                valid.add((r, c))
                for nbr, nbc in [(r + 1, c), (r - 1, c), (r, c + 1), (r, c - 1)]:
                    if nbr < 0 or nbr >= nr or nbc < 0 or nbc >= nc: continue
