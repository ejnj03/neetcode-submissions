...
class TreeMap {
    ...
    remove(key) {
        ...
        console.log(this.pairs)
        //replacement val that findReplacement will update
        let replacement = null;
        const findReplacement = (node) => {
            if (!node) {
                return null;
            }
            //find the smallest node in the right subtree of the to_delete node
            if (!node.left) {
                //record the node
                replacement = node.key;
                //cut it off (replace with its right node)
                return node.right;
            } 
            node.left = findReplacement(node.left);
        }

        const findKey = (curr_node) => {
            if (!curr_node) {
                return null;
            }
            const curr_key = curr_node.key;
            if (key < curr_key) {
               curr_node.left = findKey(curr_node.left);
            } else if (key > curr_key) {
                curr_node.right = findKey(curr_node.right);
            } else {
                ...
                if (!curr_node.left && !curr_node.right) {
                    ...
                    //replace its value   
                    curr_node.right = findReplacement(curr_node.right);
                    curr_node.key = replacement;
                    console.log("replacement: ", replacement)
                }
            }
            return curr_node;
        }
        this.root = findKey(this.root);
        console.log(this.root)
    }
